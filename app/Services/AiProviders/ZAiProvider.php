...
class ZAiProvider implements AiProviderInterface
{
    ...
    public function __construct()
    {
        ...
        $this->model = env('ZAI_MODEL', 'zai-v1');
        $this->baseUrl = env('ZAI_BASE_URL', 'https://api.zai.com/v1');
    }
...
EOT;
    ...
    {
        if (!$this->apiKey) {
            Log::error('Z.AI: API anahtarı yapılandırılmamış');
            return [
                'success' => false,
        ...
        try {
            // Base URL zaten /chat/completions içeriyorsa tekrar ekleme
            $url = str_ends_with($this->baseUrl, '/chat/completions')
                ? $this->baseUrl
                : rtrim($this->baseUrl, '/') . '/chat/completions';

            Log::debug('Z.AI: API çağrısı başlatılıyor', ['model' => $this->model, 'url' => $url]);

            $payload = $this->buildPayload($prompt, $base64Image);
            $response = Http::timeout(30)
                ->withoutVerifying() // SSL doğrulamasını geçici olarak devre dışı bırak
                ->withHeaders([
            ...
            if (!$response->successful()) {
                Log::error('Z.AI: API yanıt vermedi', [
            ...
            Log::debug('Z.AI: API yanıtı alındı');

            return $this->parseResponse($result);

        } catch (\Exception $e) {
            Log::error('Z.AI: Fiş okunamadı', [
    ...
    {
        // OpenAI uyumlu format (çoğu API bu formatı kullanır)
        return [
            'model' => $this->model,
            'messages' => [
                [
    ...
    private function parseResponse(array $response): array
    {
        try {
            ...
            Log::debug('Z.AI: Yanıt metni alındı', ['text_length' => strlen($text)]);
            ...
            if (json_last_error() === JSON_ERROR_NONE) {
                Log::info('Z.AI: JSON başarıyla ayrıştırıldı', ['data' => $data]);
                return [
                    'success' => true,
                    'data' => [
                        'date' => $data['date'] ?? null,
                ...
                Log::error('Z.AI: JSON ayrıştırma hatası', ['json_error' => json_last_error_msg()]);
            }
            ...
            Log::error('Z.AI: Yanıt işlenemedi', ['message' => $e->getMessage()]);
            return [
                'success' => false,
